...
| Layer | Technology |
|--------|------------|
| UI | React 19, Vite 7, TypeScript, **Tailwind CSS 4** (`@tailwindcss/vite`), **lucide-react** |
| Shell | Tauri 2 (Rust) |
| Plugins | `opener`, `dialog`, `notification` (JS + Rust) |
| Core CLIs | Prefer **`tools/yt-dlp.exe`** + **`tools/ffmpeg/bin/ffmpeg.exe`** next to the app (or any ancestor while dev-ing); else **`YT_DLP`** / **`FFMPEG`** env; else **`yt-dlp`** / **`ffmpeg`** on **PATH**. Bundled into NSIS/MSI via `tauri.conf.json > bundle.resources`. |
...
```
src/                    # React UI
  App.tsx               # Main shell (Tauri + Figma-style layout)
  main.tsx              # Entry + global CSS
  types.ts              # TS mirrors of payloads / settings
  contexts/ThemeContext.tsx
  styles/               # Tailwind 4 entry + theme.css, animations
  components/design/    # Migrated Figma panels + modals (VideoUrlCard, queue, settings, …)
  design/videoTypes.ts  # Shared VideoInfo for preview
src-tauri/
  src/
    ...
    thumb_proxy.rs      # Bilibili CDN thumbnails: GET with Referer → data URL
...
**Prerequisites:** Rust + MSVC (Windows), and either **bundled `tools/`** (see README) or **yt-dlp** + **ffmpeg** on PATH (or `YT_DLP` / `FFMPEG`). **`tauri build`** expects `../tools/yt-dlp.exe` and `../tools/ffmpeg/bin/` relative to `src-tauri/` when resources are enabled.
